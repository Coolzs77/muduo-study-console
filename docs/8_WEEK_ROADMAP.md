...
Week 5~6    Phase C: AI 平台业务与架构大招    贯通 MCP 工具调用、RAG 与 RabbitMQ 异步削峰
...
- **配套阅读 (1h/天)**：
  ...
  - 《鸟哥的私房菜》第 1~3 章（夯实 Linux 基础命令与环境）。
- **阶段产出 Evidence**：
...
### 第 3 周：CppAIService 之 HttpServer 协议解析与路由引擎
- **项目攻坚 (3h/天)**：
...
- **阶段产出 Evidence**：
  - `EVID-W6-01`: 对比测试“同步直接写 MySQL”与“RabbitMQ 异步削峰入库”在高并发聊天请求下的接口响应时间差异（产生震撼量化指标！）。
...
- **求职转化 (3h/天)**：
  - 激活 Career 模块的 STAR 简历生成器：
    - **S (情境)**：市面绝大多数 AI 应用框架基于 Java/Python，C++ 生态缺乏高性能、高并发的基础服务平台。
