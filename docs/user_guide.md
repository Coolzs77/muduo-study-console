...
├── index.html                 # 现代化语义主视图 HTML（约 1,257 行）
├── classic.html               # 完整单文件离线兜底版（约 7,780 行）
├── css/
...
│   ├── books-integration.js   # 权威书目物理页码偏移计算、PDF 直跳与 Readest 联动引擎（255 行）
│   ├── timer.js               # 高可靠专注计时器、防休眠漂移算法与会话自动归档（225 行）
│   └── app.js                 # 7 大视图路由、大盘指标、SVG 抽屉与持久化主业务（2,245 行）
├── docs/
...
### 2. 双权威书目深度伴读与页码直跳（全新 View 7）
- **功能入口**：点击顶部主导航第 3 个标签：<kbd><i class="fa-solid fa-book-open"></i> 书卷伴读 (Readest)</kbd>。
- **覆盖书目**（位于 `E:\workspace\C++_learning\`）：
  - **陈硕《Linux多线程服务端编程：使用muduo C++网络库》**（物理印刷偏移量 **+23** 页）
  - **《C++ Primer Plus（第6版）中文版》**（物理印刷偏移量 **+30** 页）
- **两大阅读跳转姿势**：
  - **姿势 A：极速自定义页码直跳**：
    1. 在顶部的输入框中输入实体书或教材上的印刷页码（例如 `255`）；
    2. 下拉选择对应书目（陈硕 / Primer Plus）；
    3. 点击 **【浏览器直跳】**，浏览器将直接在新标签页中打开 PDF 并自动翻至加权补偿后的绝对页码（P.278）；
    4. 点击 **【呼起 Readest】**，系统将自动唤起您本地的 Readest 客户端，同时将精准的章节与页码复制到剪贴板，并在屏幕右上角弹出提示，在 Readest 中按下 <kbd>Ctrl + G</kbd> 即可极速粘贴定位！
  - **姿势 B：章节目录与 28 天任务联动卡片**：
    在阅读视图下方，已列出陈硕全书 11 大核心章节（从线程安全的对象生命期管理、互斥器条件变量，到 Reactor 网络库源码剖析、muduo 编程实战）与 Primer Plus 经典章节对照。每一章均有快捷跳转按键，一键开启研读。

### 3. 现代 C++ 递进路线 ➔ muduo 架构拓扑图
- **功能位置**：主控制台（攻坚大盘）中段。
- **互动操作**：
  - 蓝图包含了：
